...
- 推送到 `main`：会自动生成一个**正式版 Release**
- 推送 `v*` 标签：也会自动生成一个**正式版 Release**
- 如果你不想本地打标签，也可以在 GitHub 网页里手动发布正式版

### 1. 平时开发：`push main` 自动出正式版
...
- 生成一个 **GitHub 正式 Release**
- 上传 Linux x64 发布包

自动发布的标签名会自动生成，格式类似：

```text
release-20260405-101530-90d851b
```

如果你只是想“每次新代码都自动出可下载的正式包”，用这一种就够了。
...
- 你平时只要 `git push`，就能自动拿到正式发布包
- 如果你想固定版本号，也可以继续用 `v1.0.0` 这种标签
- 不需要手动点 GitHub 页面就能自动发版
